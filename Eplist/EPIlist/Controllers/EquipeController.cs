...
public class EquipeController : ControllerBase
{
    ...
    public IActionResult CriarEquipeComUnidadeELider([FromBody] Equipe equipe)
    {
        ...
        Usuario? LiderReq = _ctx.Usuarios.FirstOrDefault(u => u.UsuarioID == equipe.LiderID && u.Cargo == "supervisor");//verificar supervisor
        if (LiderReq.EquipeID != null)
        {
            return NotFound("SeperVisor não pode estar em outra equipe");
        }
    ...
    [HttpPost]
    [Route("{equipeId}/adicionar-usuario")]
    ...
    [HttpPut]
    [Route("{equipeId}/atualizar-unidade")]
    public IActionResult AtualizarUnidadeDaEquipe(int equipeId, [FromBody] int novaUnidadeId)
    {
        var equipeExistente = _ctx.Equipes.FirstOrDefault(e => e.EquipeID == equipeId);
